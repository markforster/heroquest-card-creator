...
type AssetsInspectorActionsProps = {
  asset: AssetRecord;
  assetBlob: Blob | null;
  ...
  isOpaquePng: boolean;
  assetCount: number;
  usageBounds: AssetUsageBounds | null;
  onReplaceComplete: () => void;
  onOptimizeComplete: () => void;
  enqueueAssetReplacement: (assetId: string, dimensions: { width: number; height: number }) => void;
  onAssetBinaryUpdated?: (next: { blob: Blob; sizeBytes: number }) => void;
};

const INSPECT_ZOOM = 2;
const CONVERT_INSPECT_THRESHOLD_PX = 3;

export default function AssetsInspectorActions({
  asset,
  assetBlob,
  assetSizeBytes,
  previewUrl,
  ...
  onAssetBinaryUpdated,
}: AssetsInspectorActionsProps) {
  const { t } = useI18n();
  ...
  const [convertImageSize, setConvertImageSize] = useState<FrameSize>({ width: 0, height: 0 });
  ...
  const canReplaceImage = assetCount === 1;
  ...
  const handleInspectMove = (event: MouseEvent<HTMLDivElement>) => {
  ...
  const handleReplaceConfirm = async () => {
    ...
    try {
      ...
      enqueueAssetReplacement(asset.id, {
        width: pendingReplace.width,
        height: pendingReplace.height,
      });
      onAssetBinaryUpdated?.({
        blob: pendingReplace.file,
        sizeBytes: pendingReplace.file.size,
      });
      onReplaceComplete();
    } catch (error) {
  ...
  }, [asset.id, asset.width, asset.height, assetBlob, convertQuality, isConvertOpen, t]);
  ...
  const handleConvertApply = async () => {
    ...
    try {
      ...
      onAssetBinaryUpdated?.({
        blob: convertPreview.blob,
        sizeBytes: convertPreview.bytes,
      });
      onReplaceComplete();
      setIsConvertOpen(false);
    } catch (error) {
